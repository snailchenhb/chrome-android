...
class ChunkedTextElementAdapter extends TextElementAdapter {
    ...
    if (chunkStyle.getFont().hasSize()) {
      spannable.setSpan(
          new AbsoluteSizeSpan(
              (int) LayoutUtils.dpToPx(chunkStyle.getFont().getSize(), getContext())),
          start,
          spannable.length(),
          Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
    }
  ...
  void addImageChunk(
    ...
    int styleWidth =
        chunkStyle.hasWidth()
            ? (int) LayoutUtils.dpToPx(chunkStyle.getWidth(), getContext())
            : DIMENSION_UNKNOWN;
    int styleHeight =
        chunkStyle.hasHeight()
            ? (int) LayoutUtils.dpToPx(chunkStyle.getHeight(), getContext())
            : DIMENSION_UNKNOWN;
    getParameters()
        .hostProviders
        .getAssetProvider()
  ...
  private Drawable maybeResizeDrawable(Drawable imageDrawable, StyleProvider imageStyle) {
    if (imageStyle.hasWidth() && imageStyle.hasHeight()) {
      int styleHeight = (int) LayoutUtils.dpToPx(imageStyle.getHeight(), getContext());
      int styleWidth = (int) LayoutUtils.dpToPx(imageStyle.getWidth(), getContext());

      // Both image dimens are known.  Try to resize image down to appropriate size.
      imageDrawable =
          DrawableScalingHelper.maybeScaleDrawable(
              getContext(),
              getParameters().hostProviders.getAssetProvider(),
              imageDrawable,
              styleWidth,
              styleHeight);
    }
  ...
  class ImageSpanDrawableCallback implements Consumer</*@Nullable*/ Drawable> {
    ...
    public void accept(/*@Nullable*/ Drawable imageDrawable) {
      ...
      imageDrawable = maybeResizeDrawable(imageDrawable, imageStyle);
      checkState(
