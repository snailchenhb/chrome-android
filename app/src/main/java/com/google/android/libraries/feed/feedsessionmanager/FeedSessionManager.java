...
public class FeedSessionManager
  ...
  public FeedSessionManager(
      ...
      Clock clock,
      Observable<FeedLifecycleListener> lifecycleListenerObservable) {
  ...
  public void initialize() {
    if (initialized.get()) {
      Logger.i(TAG, "FeedSessionManager has previously been initialized");
      return;
    }
  ...
  private void populateSession(InitializableSession session) {
    ...
    if (behavior == RequestBehavior.NO_REQUEST_WITH_CONTENT && taskQueue.isMakingRequest()) {
  ...
  public void triggerRefresh(/*@Nullable*/ StreamSession streamSession) {
    ...
    taskQueue.execute(
        "triggerRefresh(Invalidate,Request)",
  ...
  public Consumer<Result<List<StreamDataOperation>>> getUpdateConsumer(
    ...
    return sessionManagerMutation.createCommitter(
        "updateConsumer",
        mutationContext,
        this::modelErrorObserver,
        sharedStateCache,
  ...
  public void onLifecycleEvent(@LifecycleEvent String event) {
    ...
    switch (event) {
      case LifecycleEvent.CLEAR_ALL:
        taskQueue.execute("reset", TaskType.IMMEDIATE, this::reset);
        break;
